...
function formatToothNumbersDisplay(toothNumbers: string[]) {
  return toothNumbers.length === 0 ? "-" : formatToothDisplay(toothNumbers.join("、"));
}
        ...
        <div className="min-h-0 flex-1">
            ...
            <label className="shrink-0 pt-2 text-xs font-semibold text-[#333333]">作業内容</label>
              ...
              <div className="flex h-10 items-center gap-5 rounded-lg border border-[#E2E2E2] bg-white px-3">
                ...
                <label className="inline-flex items-center gap-1.5 text-xs font-medium text-[#444444]">
                  <input
                    type="checkbox"
                    name="base_up_support"
                    checked={baseUpSupport}
                    onChange={(event) => setBaseUpSupport(event.target.checked)}
                    className="h-3.5 w-3.5 accent-[#fff362]"
                  />
                  ベースアップ支援金対象
                </label>
          ...
          <div className="flex h-10 items-center gap-3">
              ...
              <select
                ...
                className="h-10 w-20 shrink-0 rounded-lg border border-[#E2E2E2] bg-white px-3 text-sm font-medium text-[#333333] outline-none transition-colors focus:border-[#F0B132]"
              >
                {Array.from({ length: 10 }, (_, index) => String(index + 1)).map((quantity) => (
            ...
            <tbody>
              {confirmedDelivery.items.map((item) => (
                <tr key={item.order_item_id} className="border-b border-[#EFEFEF] text-[#2A2A2A]">
                  ...
                  <td className="px-3 py-2">{formatToothNumbersDisplay(item.tooth_numbers)}</td>
