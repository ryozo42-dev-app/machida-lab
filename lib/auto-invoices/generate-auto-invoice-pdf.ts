...
type GenerateAutoInvoicePdfInput = {
  invoiceId: number;
};

type GenerateAutoInvoicePdfResult = {
  status: "pdf_saved" | "pdf_already_exists";
  invoiceId: number;
  pdfPath: string;
  pdfFilename: string;
...
function assertPositiveInvoiceId(invoiceId: number) {
...
async function fetchInvoicePdfData(
  ...
  return {
    ...
    pdf_saved_at: invoice.pdf_saved_at,
...
async function updateInvoicePdfFields(
  invoiceId: number,
  pdfPath: string,
  pdfFilename: string,
  pdfSavedAt: Date
  ...
  await prisma.invoices.update({
    ...
    data: {
      ...
      pdf_saved_at: pdfSavedAt,
    },
  });
}

export async function generateAutoInvoicePdf({
  invoiceId,
}: GenerateAutoInvoicePdfInput): Promise<GenerateAutoInvoicePdfResult> {
  assertPositiveInvoiceId(invoiceId);
  ...
  try {
    ...
    if (storedPdf && data.pdf_path) {
      return {
        status: "pdf_already_exists",
        invoiceId: data.id,
    ...
    if (existingPdf) {
      const pdfSavedAt = new Date();

      await updateInvoicePdfFields(
        data.id,
      ...
      return {
        status: "pdf_already_exists",
        invoiceId: data.id,
        pdfPath: storageLocation.relativePath,
        pdfFilename: storageLocation.fileName,
        pdfSavedAt,
      };
    }
